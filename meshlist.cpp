...
//------------------------------------------------------------------------
void
MeshList::make_pair(std::shared_ptr<Variables> &vars, std::vector<Pair> &pairs) {
  if (!initialized) {
    sorted_buffer.resize(vars->number_of_atoms());
    initialized = true;
  }
  ...
  std::vector<int> particle_position(pn, 0);
  std::vector<int> pointer(pn, 0);
...
//------------------------------------------------------------------------
void
MeshList::search_other(int id, int ix, int iy, int iz, std::shared_ptr<Variables> &vars, std::vector<Pair> &pairs) {
  if (ix < 0) ix += m;
  if (ix >= m ) ix -= m;
  if (iy < 0) iy += m;
  if (iy >= m ) iy -= m;
...
//------------------------------------------------------------------------
void
MeshList::search(int id, std::shared_ptr<Variables> &vars, std::vector<Pair> &pairs) {
  int ix = id % m;
  int iy = (id / m) % m;
  int iz = (id / m / m);
